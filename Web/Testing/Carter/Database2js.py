...
for cluster in clusters:
    ...
    for i in range(len(row)):
        if i == 0:
            outfile.write(
                '<td class="column0"><a href="Web/Cluster_pages/{}.html" target="_blank">%s</a></td>'.format(
                    row[i]
                )
                % row[i]
            )
    ...
    with open(cluster_pages_path + "/" + cluster_name + ".html", "w+") as cluster_page:
        ...
        cluster_page.write(
            '  <button class="w3-button w3-round-large w3-teal w3-hover-purple"><a href="#Spec">Spectroscopic</a></button> &nbsp; \n'
        )
        cluster_page.write(
            '  <button class="w3-button w3-round-large w3-teal w3-hover-purple"><a href="#photo">Photometric</a></button> \n'
        )
