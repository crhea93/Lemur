...
def calc_flux(evt_file,merged_obs,energy_range,region,background,exposure = False):
    ...
    #dmextract.infile = evt_file+".fits[energy="+energy_range+"][bin sky=region("+region+".reg)]"
    dmextract.bkg = evt_file+".fits[energy="+energy_range+"][bin sky=region("+background+")]"
    ...
    dmstat.punlearn()
    dmstat.infile = obsid+'_'+energy_range3+'_thresh.expmap[sky=region('+background+')]'
    dmstat.centroid = False
    dmstat()
    ...
    if exposure == False:
        ...
        dmtcalc.punlearn()
        dmtcalc.infile = evt_file+".fits[energy="+energy_range+",sky=region("+background+")]"
    ...
    if exposure == True:
        ...
        eff2evt.punlearn()
        eff2evt.infile = evt_file+".fits[energy="+energy_range+"][sky=region("+background+")]"
...
def spec_run(directory_base,obsid,region,energy_range,bkg_file):
    ...
    specextract.infile = evt_file+'.fits[energy='+energy_range+'][sky=region('+region+'.reg)]'
    ...
    specextract()
def merge_observations(obs_to_merge,output_dir,directory_base,energy_range2,mono_energy):
    #Merge individual region files
    merging_files = ''
    for obsid in obs_to_merge:
        merging_files += directory_base+'/'+obsid+'/repro/acisf'+obsid+'_repro_evt2.fits,'
...
def CSB_calc(directory_base,dir_name,obs,cen_ra,cen_dec,bkg_file,redshift,merge_bool):
    ...
    for region in regions:
        ...
        with open(region+'.reg','w+') as new_reg:
            ...
            new_reg.write("circle("+cen_ra+','+cen_dec+','+str(reg_size)+'"'+")")
        for obsid in obs:
            try:
        ...
        create_arf(directory_base,obs,region,directory_base+'/'+dir_name+'/SurfaceBrightness')
        copyfile(region+'.reg',directory_base+'/'+dir_name+'/SurfaceBrightness/'+region+'.reg')
        ct += 1
