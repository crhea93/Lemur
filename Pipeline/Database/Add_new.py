...
def add_csb(mydb,mycursor,cluster_id,cluster_name,csb_ct,csb_ct_l,csb_ct_u,csb_pho,csb_pho_l,csb_pho_u,csb_flux,csb_flux_l,csb_flux_u):
    '''
    Add information to cluster table
    :param mydb: my database name
    :param mycursor: cursor name
    ...
    if number_of_rows == 0:
        ...
        val = (cluster_name,cluster_id,csb_ct,csb_ct_l,csb_ct_u,csb_pho,csb_pho_l,csb_pho_u,csb_flux,csb_flux_l,csb_flux_u)
        mycursor.execute(sql, val)
    else:
        #Cluster exists
        ...
        val = (csb_ct,csb_ct_l,csb_ct_u,csb_pho,csb_pho_l,csb_pho_u,csb_flux,csb_flux_l,csb_flux_u,cluster_name)
        mycursor.execute(sql, val)
    mydb.commit()
    return None

def add_r_cool(mydb,mycursor,cluster_id,cluster_name,R_cool_3,R_cool_3_l,R_cool_3_u,R_cool_7,R_cool_7_l,R_cool_7_u):
    '''
    Add information to cluster table
    :param mydb: my database name
    :param mycursor: cursor name
    ...
    if number_of_rows == 0:
        ...
        val = (cluster_id,cluster_name,str(R_cool_3),str(R_cool_3_l),str(R_cool_3_u),str(R_cool_7),str(R_cool_7_l),str(R_cool_7_u))
    else:
        sql = "UPDATE r_cool SET R_cool_3= %s,R_cool_3_l= %s,R_cool_3_u= %s, R_cool_7=%s,R_cool_7_l=%s,R_cool_7_u=%s WHERE ID = %s"
        val = (str(round(R_cool_3,2)), str(round(R_cool_3_l,2)),str(round(R_cool_3_u,2)),str(round(R_cool_7,2)),str(round(R_cool_7_l,2)),str(round(R_cool_7_u,2)), cluster_id)
    mycursor.execute(sql, val)
    mydb.commit()
    return None
...
def add_obsid_db(mydb,mycursor,cluster_name,obsid):
    ...
    mycursor.nextset()
    #Check if obsid is already associated with cluster id in obsids table
    mycursor.execute("SELECT COUNT(*) FROM Obsids Where ClusterNumber = %s AND Obsid=%s",(id,str(obsid)))
    (number_of_rows,) = mycursor.fetchone()
    # gets the number of rows affected by the command executed
    mycursor.nextset()
    if number_of_rows == 0:
...
def add_fit_db(mydb,mycursor,clust_name,reg_id,r_in,r_out,area,temp,temp_min,temp_max,abund,ab_min,ab_max,norm,norm_min,norm_max,flux,redchisq,agn_):
    '''
    ...
    if number_of_rows == 0:
        ...
        val = (id,int(reg_id),r_in,r_out,area,temp,temp_min,temp_max,abund,ab_min,ab_max,norm,norm_min,norm_max,flux,redchisq,agn_)
        mycursor.execute(sql, val)
    else:
        #Cluster exists
        ...
        val = (r_in,r_out,area,temp,temp_min,temp_max,abund,ab_min,ab_max,norm,norm_min,norm_max,flux,redchisq,agn_,id,int(reg_id))
        mycursor.execute(sql, val)
    mydb.commit()
    return None

def add_fit_additional_db(mydb,mycursor,clust_name,reg_id,Lum,dens,dens_min,dens_max,press,press_min,press_max,entr,entr_min,entr_max,t_cool,t_cool_min,t_cool_max):
    '''
    Add fit parameters to database containing regions for each cluster
    :param clust_name: name of cluster
    :param reg_id: id of region (annulus)
    ...
    val = (Lum,dens,dens_min,dens_max,press,press_min,press_max,entr,entr_min,entr_max,t_cool,t_cool_min,t_cool_max,id,int(reg_id))
    mycursor.execute(sql, val)
    mydb.commit()
    return None
