...
def all_profiles(mydb, mycursor, data_folder, output_folder, redshift, cluster_id):
    ...
    """
    properties = {
        "Temperature": [["Temp", "Temp_min", "Temp_max"], "keV"],
        "Abundance": [["Abundance", "Ab_min", "Ab_max"], "$Z/Z_{\odot}$"],
        "Density": [["Density", "Dens_min", "Dens_max"], "$cm^{-3}$"],
        "Pressure": [["Pressure", "Press_min", "Press_max"], "$erg cm^{-3}$"],
        "Entropy": [["Entropy", "Entropy_min", "Entropy_max"], "$keV cm^{2}$"],
        "T_Cool": [["T_cool", "T_cool_min", "T_cool_max"], "Gyr"],
    }
    for key, val in properties.items():
        profile(
            mydb, mycursor, data_folder, output_folder, redshift, cluster_id, key, val
        )
    return None


def profile(
    mydb, mycursor, data_folder, output_folder, redshift, cluster_id, property, units
):
    """
    ...
    sql_select_Query = (
        "select "
        + props
        + "AGN,R_in,R_out from Region WHERE idCluster=%s ORDER BY idRegion"
        % (cluster_id)
    )
