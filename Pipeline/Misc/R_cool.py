...
def R_cool_calc(
    mydb, mycursor, cluster_id, cluster_name, data_folder, redshift, main_out
):
    sql_select_Query = (
        "select T_cool,T_cool_min,T_cool_max,AGN,R_in,R_out from Region WHERE idCluster=%s ORDER BY idRegion"
        % (cluster_id)
    )
